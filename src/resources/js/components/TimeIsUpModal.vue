...
import { TriangleAlert, Bell } from "lucide-vue-next";
                            ...
                            <DialogPanel class="relative transform overflow-hidden rounded-lg bg-gray-800 text-left shadow-xl outline -outline-offset-1 outline-white/10 transition-all sm:my-8 sm:w-full sm:max-w-lg">
                                <div class="bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                    <div class="sm:flex sm:items-start">
                                        <div class="mx-auto flex size-12 shrink-0 items-center justify-center rounded-full bg-yellow-500/10 sm:mx-0 sm:size-10">
                                            <Bell class="size-8 text-yellow-400 animate-bounce" aria-hidden="true" />
                                        </div>
                                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                            <DialogTitle as="h3" class="text-base font-semibold text-white">Tiden er gået!</DialogTitle>
                                            <div class="mt-2 text-sm text-gray-400">
                                                <p>Tiden er gået, tryk på 'forsæt' for at lukke vinduet</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="bg-gray-700/25 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                    <button @click="$emit('close', true)" type="button" class="mt-3 inline-flex w-full justify-center rounded-md bg-white/10 px-3 py-2 text-sm sm:ml-3 font-semibold text-white inset-ring inset-ring-white/5 hover:bg-white/20 sm:mt-0 sm:w-auto" ref="cancelButtonRef">Cancel</button>
                                    <slot name="action"/>
                                </div>
                            </DialogPanel>
                        </TransitionChild>
